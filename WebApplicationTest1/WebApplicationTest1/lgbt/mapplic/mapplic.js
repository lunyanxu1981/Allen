...
(function($) {
	...
	var Mapplic = function() {
		...
		self.o = {
			...
			skin: '',
			zoom: true,
			showFullSelect: false,
			showLevelHeadline: false,
		...
		self.init = function(el, params) {
			...
			self.o.selectedCategories = [];
			self.o.selectedKanton = undefined;

			$('.mapplic-activate-category').on('click', function(ev){

				if ( $(ev.currentTarget).hasClass('mapplic-isactivated') ){
					$(ev.currentTarget).removeClass('mapplic-isactivated');
				} else {
					$(ev.currentTarget).addClass('mapplic-isactivated');
				}

				self.o.selectedCategories = [];
				$('.mapplic-activate-category.mapplic-isactivated').each( function( index, element ){
					var name = $(element).attr('mapplic-category');
					self.o.selectedCategories.push( name.toLowerCase() );
				});
			...
			if (typeof self.o.source === 'string') {
				...
				$.getJSON(source, function(data) { // Success
					processData(data);
		...
		function refreshCategories(){
			...
			var selectedKanton = undefined;

			if ( $('.st0-1').length == 1 ){
				selectedKanton = $('.st0-1').parent().parent().attr('id').toLowerCase();
			}



			$(self.o.selectedCategories).each( function(index, elem ){

				// wenn wir noch ein Kanton haben auch danach checken
				if ( selectedKanton != undefined ){

					
					$('[data-category='+elem+'][data-kanton='+selectedKanton+']').show();
				} else {
					$('[data-category='+elem+']').show();	
				}
		...
		function Tooltip() {
			...
			this.init = function() {
				...
				if ( typeof self.o.source_dark === 'string') this.el.addClass('dark');
				...
				this.title = $('<div></div>').addClass('mapplic-tooltip-title').appendTo(this.el);
				...
				if ( self.o.tooltipBindOuter != false ){
					$( self.o.tooltipBindOuter ).append( this.el );
				} else self.map.append(this.el);
			...
			this.show = function(location) {
				if (location) {
					...
					var elementSub = this.el;
					var classList = this.el.attr('class').split(/\s+/);
					$.each(classList, function(index, item) {
					    if ( item.startsWith("tooltip_cat_")) {
					        elementSub.removeClass( item );
					    }
					});
					this.el.addClass( 'tooltip_cat_'+location.category );
			...
			this.position = function() {
				if (this.location) {
					...
					var x = this.location.x * 100,
						...
						var adaptive_k = 100;
						if ( window.innerWidth < 660 && Math.abs(mt) > cy ) {
							adaptive_k = 30;
						}
						y = this.location.y * adaptive_k;
		...
		function HoverTooltip() {
			...
			this.init = function() {
				...
				if ( self.o.source_dark ) this.el.addClass('dark');
				this.title = $('<div></div>').addClass('mapplic-tooltip-title').appendTo(this.el);
			...
			this.show = function(location) {
				...
				if (self.tooltip.location != location) {
					...
					//this.title.html( location.title );
					this.title.html( location.titleHover );
					...
					/*this.title.append( location.icon);
					this.title.append('<p>'+location.about+'</p>');*/
		...
		var processData = function(data) {
			...
			if ( self.o.showFullSelect == true ){
				self.levelselect.attr('size', data.levels.length+1 );
				self.levelselect.css({'overflow':'hidden','padding-top':'0.25rem','padding-bottom':'0.25rem'});
			}
						...
						case 'svg':
							$('<div></div>').addClass('mapplic-map-image').load(source, function() {
								...
								$(this).on('click',function(e){
									...
									if ( $(e.target).attr('class') != 'mapplic-active' && self.o.hideOnClickOutside == true ){
										e.preventDefault();
										$('.mapplic-active', self.el).attr('class', 'mapplic-clickable');
										if (self.deeplinking) self.deeplinking.clear();
										if (!self.o.zoom) zoomTo(0.5, 0.5, 1, 600, 'easeInOutCubic');
								...
								$('.mapplic-all-kantons').on('click', function( e ){

									console.log('sddsfsd');
									$('.st0').attr('class', 'st0-1' );	
								...
								$('svg .st0', this ).click( function(e){
									...
									if ( self.o.drags < 5 ){
										...
										// wenn 
										if ( $('.st0-1').length == 1 && $(this).attr('class') == 'st0-1' )	{
											$('.st0').attr('class', 'st0-1' );										

										} else {

											$('.st0-1').attr('class', 'st0' );
											$(this).attr('class', 'st0-1' );	
										}
					...
					$.each(value.locations, function(index, value) {
						...
						if (value.pin != 'hidden') {
							if (self.o.markers) {
								...
								pin.attr('data-kanton', value.kanton.toLowerCase() );
								pin.attr('data-category', value.category.toLowerCase() );
			...
			if (nrlevels > 1) {
				...
				if ( self.o.showLevelHeadline ){

					self.levelheadline = $('<h1>ddd</h1>').addClass('mapplic-levels-headline');
					self.container.append( self.levelheadline );
			...
			shownLevel = $(self.levelselect.children('option')[0]).val();
		...
		var switchLevel = function(target, tooltip) {
			...
			if ( self.o.showLevelHeadline == true ){

				self.levelheadline.html(  $('option:selected', self.levelselect).html()  );
