...
class ThumbHandler
{
    ...
    function setSrcImg($src_img, $img_type=null)
    {
        ...
        $this->h_src = $this->imageCreateFrom($src_img);
    ...
    {
    	return !empty($this->src_info) ? $this->src_info : $this->src_info = @getimagesize($src);
    }
    
    function imageCreateFrom($src)
    {
    	$info = $this->getSrcInfo($src);
    	$mime = $info['mime'];
    	list($ks, $type) = explode('/', $mime);
    ...
    function resize($src, $w, $h)
    {
        ...
        imagecopyresampled($new_im, $src, 0, 0, 0, 0, $w, $h, $src_w, $src_h);
        $this->h_dst = $new_im;
    }
    ...
    function _output()
    {
        ...
        if(function_exists($func_name))
        {
            ...
            if ($func_name == 'imagepng') $this->img_display_quality = 8;//php5.1.2以上版本的imagepng函数参数范围为0-9
