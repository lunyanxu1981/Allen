...
class RecommendModel extends Model
{
	...
	private $fieldStr = "|id|type|category|title|desc|aid|img_name|url|rank|status|is_del|delete_time|update_time|create_time";
	...
	public function getItemList($whereArr,$limit,$orderBy){
		...
		if (!empty($itemList)){
			...
			foreach ($itemList as $key=>$val){
				...
				$itemList[$key]["httpPostImg"] = empty($val["img_name"]) ? "" : "https://".MY_VAN_DOMAIN."/images/upload/recommend/".$val["img_name"];
	...
	public function getOneItem($whereArr){
		...
		if (!empty($itemArr)) {
			$itemArr["httpPostImg"] = empty($itemArr["img_name"]) ? "" : "https://".MY_VAN_DOMAIN."/images/upload/recommend/".$itemArr["img_name"];
