...
{    
    ...
    public function index(){
		...
		*/
		...
		$itemObjR = Factory::loadModel("Notice");
		...
		return View::parse("notice/notice_index.html",$data);
    }
	/**
	 * 新增
	 */
    public function add(){
		...
		if ($this->request->isPostBack()) {
			...
			if($_FILES['img_arr']['name'] != ""){
				...
				//建文件夹
				$photoDir = "../Web/images/upload/notice/".date("Ym");
			...
			*/
			...
			$itemObjW = Factory::loadModel("Notice",$master);
			$itemId = $itemObjW->addItem($postArr,$upFileArr);
			js_show("alert('新增成功！');parent.location.href='/notice_index.html'");
			exit();
		}
		...
		$data['header'] = PublicService::getHeaderHtmlBackend();
		$data['footer'] = PublicService::getFooterHtmlBackend();
		return View::parse("notice/notice_add.html",$data);
    }
	/**
	 * 编辑
	 */
	public function edit(){
		...
		$data['header'] = PublicService::getHeaderHtmlBackend();
		$data['footer'] = PublicService::getFooterHtmlBackend();
		return View::parse("notice/notice_edit.html",$data);
	}
	/**
	 * 接口名称：删除项目
	 * URL：	/notice_del.html
