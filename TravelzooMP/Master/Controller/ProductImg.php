...
{    
    ...
    public function index(){
		...
		/*
		$searchArr["item"] = $searchItem = $getArr["item"];
		$searchArr["txt1"] = $searchTxt1 = $getArr["txt1"];
		$searchArr["txt2"] = $searchTxt2 = $getArr["txt2"];
		$data["searchArr"] = $searchArr;
		if($searchItem == "ip"){
			$whereArr .= " AND ip='$searchTxt1'";
		}elseif($searchItem == "country"){
			$whereArr .= " AND country='$searchTxt1'";
		}elseif($searchItem == "area"){
			$whereArr .= " AND area='$searchTxt1'";
		}elseif($searchItem == "createTime"){
			$whereArr .= " AND date(create_time)>='$searchTxt1' AND date(create_time)<='$searchTxt2'";
		}
		*/

		$itemObjR = Factory::loadModel("ProductImg");
		$itemList = $itemObjR->getItemList($whereArr,($nowPage-1)*$perPage.",".$perPage,"pid DESC,rank ASC,id ASC");
		...
		return View::parse("product/productImg_index.html",$data);
    }
	/**
	 * 新增
	 */
    public function add(){
		...
		if ($this->request->isPostBack()) {
			...
			$itemObjW = Factory::loadModel("ProductImg",$master);
			$itemId = $itemObjW->addItem($postArr,$upFileArr);
			js_show("alert('新增成功！');parent.location.href='/productImg_index.html?pid={$postArr["pid"]}'");
			exit();
		}
		...
		$data['header'] = PublicService::getHeaderHtmlBackend();
		$data['footer'] = PublicService::getFooterHtmlBackend();
		return View::parse("product/productImg_add.html",$data);
    }
	/**
	 * 编辑
	 */
	public function edit(){
		...
		$data['header'] = PublicService::getHeaderHtmlBackend();
		$data['footer'] = PublicService::getFooterHtmlBackend();
		return View::parse("product/productImg_edit.html",$data);
	}
	/**
	 * 接口名称：删除项目
	 * URL：	/productImg_del.html
	 * 方式：	POST
	 * 参数：	eid
