...
{    
    ...
    public function index(){
		...
		*/
		...
		$itemObjR = Factory::loadModel("UserFeedback");
		...
		return View::parse("user/userFeedback_index.html",$data);
    }
	/**
	 * 新增
	 */
    public function add(){
		...
		if ($this->request->isPostBack()) {
			...
			if($_FILES['img_arr']['name'] != ""){
				...
				//建文件夹
				$photoDir = "../Web/images/upload/userFeedback/".date("Ym");
			...
			*/
			...
			$itemObjW = Factory::loadModel("UserFeedback",$master);
			$itemId = $itemObjW->addItem($postArr,$upFileArr);
			js_show("alert('新增成功！');parent.location.href='/userFeedback_index.html'");
			exit();
		}
		...
		$data['header'] = PublicService::getHeaderHtmlBackend();
		$data['footer'] = PublicService::getFooterHtmlBackend();
		return View::parse("user/userFeedback_add.html",$data);
    }
	/**
	 * 编辑
	 */
	public function edit(){
		...
		$data['header'] = PublicService::getHeaderHtmlBackend();
		$data['footer'] = PublicService::getFooterHtmlBackend();
		return View::parse("user/userFeedback_edit.html",$data);
	}
	/**
	 * 接口名称：删除项目
	 * URL：	/userFeedback_del.html
	 * 方式：	POST
	 * 参数：	eid
