...
{    
    ...
    public function index(){
		...
		*/
		...
		$itemObjR = Factory::loadModel("BannerList");
		...
		return View::parse("product/bannerList_index.html",$data);
    }
	/**
	 * 新增
	 */
    public function add(){
		...
		if ($this->request->isPostBack()) {
			...
			if($postArr["status"]=="1"){
				$pidArr = explode("\n", $postArr["pid_str"]);
				$tNum = count($pidArr);
				//先清空该分类下的产品，再更新产品表中的banner_id值
				$productModel = Factory::loadModel("Product");
				$productModel->updateList("banner_id=0","banner_id='{$postArr["type"]}'");
				foreach ($pidArr as $pid) {
					if ($pid) {
						$productModel->updateOne("banner_id='{$postArr["type"]}',banner_rank=$tNum","Id='$pid'");
					}
					$tNum=$tNum-1;
				}
			}
			...
			$itemObjW = Factory::loadModel("BannerList",$master);
			$itemId = $itemObjW->addItem($postArr,$upFileArr);
			js_show("alert('新增成功！');parent.location.href='/bannerList_index.html'");
			exit();
		}
		//banner列表
		...
		$data['header'] = PublicService::getHeaderHtmlBackend();
		$data['footer'] = PublicService::getFooterHtmlBackend();
		return View::parse("product/bannerList_add.html",$data);
    }
	/**
	 * 编辑
	 */
	public function edit(){
		...
		$data['header'] = PublicService::getHeaderHtmlBackend();
		$data['footer'] = PublicService::getFooterHtmlBackend();
		return View::parse("product/bannerList_edit.html",$data);
	}
	/**
	 * 接口名称：删除项目
	 * URL：	/bannerList_del.html
	 * 方式：	POST
	 * 参数：	eid
